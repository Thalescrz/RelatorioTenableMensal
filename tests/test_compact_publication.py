...
class CompactPublicationTests(unittest.TestCase):
    ...
    def test_component_retry_publishes_new_immutable_snapshot_revision(self) -> None:
        profile = load_client_profile(ROOT / "clients/examples/client-profile.json")
        period = explicit_reporting_period(
            start_at="2026-07-01T00:00:00-03:00",
            end_at="2026-08-01T00:00:00-03:00",
        ...
        with tempfile.TemporaryDirectory() as directory_name:
            ...
            base = directory / "base.docx"
            cloud = directory / "cloud.docx"
            base.write_bytes(b"base")
            cloud.write_bytes(b"cloud")
            first = publish_compact_run_snapshot(
                repository=repository,
                profile=profile,
                run_id="published-run",
                execution_type="MANUAL",
            ...
            revised = publish_compact_run_snapshot(
                repository=repository,
                profile=profile,
                run_id="published-run",
                snapshot_run_id="published-run-component-retry-2",
                execution_type="MANUAL",
                period=period,
                output_root=directory,
                document_references={
                    "base": str(base),
                    "cloud:expanded": str(cloud),
                },
                publication_validated=True,
                documents_validated=True,
            )
        ...
        self.assertEqual(revised.run_id, "published-run-component-retry-2")
